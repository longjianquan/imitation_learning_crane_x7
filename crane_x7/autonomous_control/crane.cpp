...
int CR7::getCranex7JointState()
{
  ...
  // set bulk read parameter (present positon, present velosity, present
current) for (int i = 0; i < JOINT_NUM2; i++)
  {
    dxl_addparam_result = groupBulkReadAddParam(groupBulkRead, ID[i],
PRESENT_VALUE_ADDRESS, PRESENT_VALUE_DATA_LENGTH); if (dxl_addparam_result !=
True)
    {
      fprintf(stderr, "[ID:%03d] groupBulkRead addparam failed", ID[i]);
      return 1;
  ...
  if ((dxl_comm_result = getLastTxRxResult(portHandler, PROTOCOL_VERSION)) !=
COMM_SUCCESS) printf("%s\n", getTxRxResult(PROTOCOL_VERSION, dxl_comm_result));

  // verification of received data
  for (int i = 0; i < JOINT_NUM2; i++)
  {
    dxl_getdata_result = groupBulkReadIsAvailable(groupBulkRead, ID[i],
PRESENT_VALUE_ADDRESS, PRESENT_VALUE_DATA_LENGTH); if (dxl_getdata_result !=
True)
    {
  ...
  for (int i = 0; i < JOINT_NUM2; i++)
  {
    dxl_present_position = groupBulkReadGetData(groupBulkRead, ID[i],
PRESENT_POSITION_ADDRESS, PRESENT_POSITION_DATA_LENGTH);
...
int CR7::setCranex7Torque(double *torque_array,
  ...
  // fprintf( ffp,
  // "%d,%d,%d,%d,%d,%d,%d,%d,",goal_current[0],goal_current[1],goal_current[2],goal_current[3],goal_current[4],goal_current[5],goal_current[6],goal_current[7]);
  for (int k = 0; k < JOINT_NUM2; k++) {
    param_goal_current[0] = DXL_LOBYTE(DXL_LOWORD(
        (goal_current[k])));  //�ʐM�p�Ƀf�[�^�𕪂���
    ...
    dxl_addparam_result = groupBulkWrite->addParam(
        ID[k], GOAL_CURRENT_ADDRESS, GOAL_CURRENT_DATA_LENGTH,
        param_goal_current);  //�������ݗp�̃p�P�b�g�ɒǉ�
...
bool CR7::Setoperation(int Operationmode, int ID[JOINT_NUM]) {
  ...
  for (int i = 0; i < JOINT_NUM2; i++)
    dxl_comm_result = packetHandler->write1ByteTxRx(
        portHandler, ID[i], OPERATING_MODE_ADDRESS, Operationmode,
...
void CR7::Move_Goal_Position(double *goal_pose, int ID[JOINT_NUM],
  ...
  param_goal_position[0] = DXL_LOBYTE(DXL_LOWORD(rad2dxlvalue(goal_pose[i])));
//通信用にデータを分ける param_goal_position[1] =
DXL_HIBYTE(DXL_LOWORD(rad2dxlvalue(goal_pose[i]))); param_goal_position[2] =
DXL_LOBYTE(DXL_HIWORD(rad2dxlvalue(goal_pose[i]))); param_goal_position[3] =
DXL_HIBYTE(DXL_HIWORD(rad2dxlvalue(goal_pose[i])));

  dxl_addparam_result = groupBulkWrite->addParam(ID[i], GOAL_POSITION_ADDRESS,
GOAL_POSITION_DATA_LENGTH, param_goal_position);
//書き込み用のパケットに追加 if(dxl_addparam_result != true) printf("goal pose
error!\n");
